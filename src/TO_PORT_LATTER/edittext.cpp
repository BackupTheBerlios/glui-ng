/****************************************************************************
  ...
  ---------------------------
     ...
     glui_edittext.cpp - EditText control class
...
/////////////////////////////////////////////////////////////////
void EditText::common_init( void ) {
...
EditText::EditText( Node *parent, const char *name,
                              char *live_var,
                              int id, CB callback ):
    Container(name)

{
  common_construct( parent, name,  LIVE_TEXT, live_var, id, callback);
}
...
{
  common_construct( parent, name, LIVE_STRING, &live_var, id, callback);
}

/****************************** EditText::common_construct() **********/

void EditText::common_construct( Node *parent, const char *name,
                                      ...
                                      CB cb )
{
  ...
  text = new Text (this->glui->font);

  init_live();
}

/****************************** EditText::mouse_down_handler() **********/

int    EditText::mouse_down_handler( int local_x, int local_y )
{
...
/******************************** EditText::mouse_up_handler() **********/

int    EditText::mouse_up_handler( int local_x, int local_y, bool inside )
{
  return false;
}


/***************************** EditText::mouse_held_down_handler() ******/

int    EditText::mouse_held_down_handler( int local_x, int local_y,
					       bool new_inside)
{
  int tmp_pt;
  ...
  if ( tmp_pt == -1 && sel_end != 0 ) {    /* moved mouse past left edge */
    special_handler( GLUT_KEY_LEFT, GLUT_ACTIVE_SHIFT );
  }
  else if ( tmp_pt == substring_end+1 && sel_end != (int) text->length()) {
    /* moved mouse past right edge */
    special_handler( GLUT_KEY_RIGHT, GLUT_ACTIVE_SHIFT );
  }
  else if ( tmp_pt != -1 && tmp_pt != sel_end ) {
...
/****************************** EditText::key_handler() **********/

int    EditText::key_handler( unsigned char key,int modifiers )
{
  int i, regular_key;
  /* int has_selection;              */
  ...
  regular_key = false;
  bool ctrl_down = (modifiers & ControlMask)!=0;
  ...
  else if ( (key == 127 && !ctrl_down) OR  /* FORWARD DELETE */
            ( key == CTRL('d') && modifiers == ControlMask) )
  {
    if ( sel_start == sel_end ) {   /* no selection */
      if ( insertion_pt < (int)text->length() ) {
        /*** See if we're deleting a period in a float data-type box ***/
        if ( data_type == GLUI_EDITTEXT_FLOAT && (*text)[insertion_pt]=='.' )
          num_periods--;
  ...
  else if ( ((key == 127) && ctrl_down) OR   // Delete word forward
            ((key == 'd') && (modifiers == Mod1Mask)) )
  {
    if ( sel_start == sel_end ) {   /* no selection */
    ...
    if ( sel_start == sel_end ) {   /* no selection */
      if ( insertion_pt > 0 ) {
        /*** See if we're deleting a period in a float data-type box ***/
        if ( data_type == GLUI_EDITTEXT_FLOAT && (*text)[insertion_pt-1]=='.' )
          num_periods--;
  ...
  else if ( modifiers == ControlMask )  /* CTRL ONLY */
  {
    /* Ctrl-key bindings */
    if ( key == CTRL('a') ) {
  ...
  else if ( modifiers == Mod1Mask ) /* ALT ONLY */
  {
    if ( key == 'b' ) { // Backward word
      return special_handler ( GLUT_KEY_LEFT, ControlMask );
    }
    if ( key == 'f' ) { // Forward word
      return special_handler ( GLUT_KEY_RIGHT, ControlMask );
    }
  }
  else if ( (modifiers & ControlMask) OR
            (modifiers & Mod1Mask) )
  {
  ...
  draw_text_only =
  (! substring_change && ! has_selection && regular_key );
  */
...
void    EditText::activate( int how )
{
  if ( debug )
...
/************************** EditText::update_substring_bounds() *********/

int    EditText::update_substring_bounds( void )
{
  int box_width;
  ...
  if ( insertion_pt >= 0 &&
	...
	substring_end--;
      ...
      while(substring_end < text_len-1
            && substring_width( substring_start, substring_end ) <= box_width)
      	substring_end++;
    }
  }
...
/********************************* EditText::update_x_offsets() *********/

void    EditText::update_x_offsets( void )
{
}
...
void    EditText::draw_text( int x, int y )
{
  GLUI_DRAWINGSENTINAL_IDIOM
...
int  EditText::find_insertion_pt( int x, int y )
{
  int curr_x, i;
...
/******************************** EditText::draw_insertion_pt() *********/

void     EditText::draw_insertion_pt( void )
{
  int curr_x, i;
...
/******************************** EditText::substring_width() *********/

int  EditText::substring_width( int start, int end )
{
  int i, width;
...
/***************************** EditText::update_and_draw_text() ********/

void   EditText::update_and_draw_text( void )
{
  if ( ! can_draw() )
    return;
...
/********************************* EditText::special_handler() **********/

int    EditText::special_handler( int key,int modifiers )
{
  if ( ! glui )
    return false;
...
int    EditText::find_word_break( int start, int direction )
{
  int    i, j;
  char   *breaks = " :-.,";
...
/********************************** EditText::clear_substring() ********/

void   EditText::clear_substring( int start, int end )
{
  int i;
...
void   EditText::update_size( void )
{
  int delta;
...
void    EditText::set_text( const char *new_text )
{
...
/******************************* EditText::set_float_val() ************/

void   EditText::set_float_val( float new_val )
{
...
/********************************** EditText::set_int_val() ************/

void   EditText::set_int_val( int new_val )
{
  if ( has_limits == GLUI_LIMIT_CLAMP ) {
    /*** Clamp the new value to the existing limits ***/
...
/********************************* EditText::set_float_limits() *********/

void EditFloatText::set_float_limits( float low, float high, int limit_type )
{
  has_limits  = limit_type;
  float_low   = low;
  float_high  = high;

  if ( ! IN_BOUNDS( float_val, float_low, float_high ))
...
/*********************************** EditText::set_int_limits() *********/

void   EditIntText::set_int_limits( int low, int high, int limit_type )
{
  has_limits  = limit_type;
  int_low     = low;
  int_high    = high;

  if ( ! IN_BOUNDS( int_val, int_low, int_high ))
    set_int_val( int_low );
...
void    EditText::set_numeric_text( void )
{
  char buf_num[200];
  int  i, text_len;
...
void   EditText::dump( FILE *out, const char *name )
{
  fprintf( out,
...
int    EditText::mouse_over( int state, int x, int y )
{
  if ( state ) {
