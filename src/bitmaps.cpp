...
#warning "remove this huggly stuff and move them to corresponding classes"
...
void Bitmap::init_grey(unsigned char *array)
{
	w = array[0]; h = array[1];
...
/*********************************** StdBitmaps::draw() *****************/
StdBitmaps* StdBitmaps::Instance()
{
	static StdBitmaps bitmaps;
	return &bitmaps;
}
...
int StdBitmaps::width(Codes i) const
{
	assert(i>=0 && i<NUM_ITEMS);
	return bitmaps[i].w;
}

int StdBitmaps::height(Codes i) const
{
	assert(i>=0 && i<NUM_ITEMS);
	return bitmaps[i].h;
}

void StdBitmaps::draw(Codes i, int x, int y) const
{
	assert(i>=0 && i<NUM_ITEMS);
